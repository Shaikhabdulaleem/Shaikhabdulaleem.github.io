    ...
    <div className="mt-3 grid gap-2">
      {actions.map((action) => (
        <a
          ...
          className={`flex min-h-11 items-center justify-center rounded-lg border px-3 py-2 text-center text-xs font-semibold transition-colors ${action.tone === 'green' ? 'border-emerald-400/30 bg-emerald-500/10 text-emerald-200 hover:bg-emerald-500/20' : 'border-purple-400/30 bg-purple-500/10 text-purple-200 hover:bg-purple-500/20'}`}
        >
          {action.label} →
        </a>
      ))}
...
export default function PortfolioChat() {
  ...
  const chatHistoryRef = useRef(messages);
  ...
  useEffect(() => {
    if (isOpen) messagesRef.current?.scrollTo({ top: messagesRef.current.scrollHeight, behavior: reduceMotion ? 'auto' : 'smooth' });
  ...
  const sendMessage = async (rawValue) => {
    ...
    const fallback = createDeterministicResponse(value, currentState);
    abortRef.current = new AbortController();
    let result = fallback;
    try {
      result = await requestAssistant(value, history, currentState, abortRef.current.signal);
    } catch (error) {
    ...
    setMessages((current) => {
      ...
      chatHistoryRef.current = nextMessages;
      return nextMessages;
    });
          ...
          <motion.section ref={panelRef} role="dialog" aria-modal="true" aria-labelledby="portfolio-chat-title" onKeyDown={handleKeys} initial={{ opacity: 0, y: 18, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 12, scale: 0.98 }} className="mb-3 flex h-[min(720px,calc(100vh-7rem))] w-[calc(100vw-2rem)] flex-col overflow-hidden rounded-2xl border border-cyan-500/25 bg-[#0B1020]/95 shadow-[0_24px_80px_rgba(0,0,0,0.55),0_0_50px_rgba(34,211,238,0.08)] backdrop-blur-xl sm:w-[420px]">
            <header className="flex items-center justify-between border-b border-gray-800 px-4 py-3.5">
              <div className="flex items-center gap-3">
                <div className="flex h-10 w-10 items-center justify-center rounded-full border border-cyan-400/40 text-xs font-black text-cyan-200">SH</div>
                <div><h2 id="portfolio-chat-title" className="text-sm font-semibold text-white">Portfolio Assistant + Project Agent</h2><p className="mt-0.5 text-[11px] text-emerald-300">Verified portfolio facts · guided project discovery</p></div>
              </div>
              <div className="flex items-center"><button type="button" onClick={reset} className="min-h-11 rounded-lg px-2 text-xs text-gray-400 hover:text-white">Reset</button><button type="button" onClick={closeChat} aria-label="Close assistant" className="h-11 w-11 rounded-lg text-2xl text-gray-400 hover:text-white">×</button></div>
            </header>
              ...
              {messages.map((message) => (
                <div key={message.id} className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[90%] rounded-2xl px-3.5 py-3 ${message.role === 'user' ? 'rounded-br-md bg-gradient-to-br from-cyan-500 to-blue-600 text-white' : 'rounded-bl-md border border-gray-800 bg-[#151B2C] text-gray-300'}`}>
                    ...
                    {message.entries?.length > 0 && <div className="mt-3 flex flex-wrap gap-2">{message.entries.slice(0, 3).map((entry) => <a key={entry.id} href={entry.href} onClick={closeChat} className="flex min-h-10 items-center rounded-full border border-cyan-500/25 px-3 py-1 text-[11px] text-cyan-200">View {entry.title}</a>)}</div>}
                    <MessageActions actions={message.actions} />
                  </div>
                </div>
              ))}
              {loading && <div className="motion-safe:animate-pulse text-[11px] text-cyan-300">Checking the portfolio knowledge…</div>}
            </div>

            <div className="border-t border-gray-800 bg-[#0E1322] px-4 py-3">
              {suggestions.length > 0 && <div className="portfolio-chat-suggestions flex gap-2 overflow-x-auto pb-3">{suggestions.map((suggestion) => <button key={suggestion} type="button" onClick={() => sendMessage(suggestion)} className="min-h-10 shrink-0 rounded-full border border-gray-700 bg-[#151B2C] px-3 py-1.5 text-[11px] text-gray-300 hover:border-cyan-500/40 hover:text-cyan-200">{suggestion}</button>)}</div>}
              <form onSubmit={(event) => { event.preventDefault(); sendMessage(input); }} className="flex gap-2"><label htmlFor="portfolio-chat-input" className="sr-only">Ask the portfolio assistant</label><input ref={inputRef} id="portfolio-chat-input" value={input} onChange={(event) => setInput(event.target.value)} maxLength={1000} placeholder="Ask about services or describe your project…" className="min-w-0 flex-1 rounded-xl border border-gray-700 bg-[#141B2D] px-3.5 py-3 text-sm text-white outline-none focus:border-cyan-500" /><button disabled={!input.trim() || loading} className="min-h-11 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-40">Send</button></form>
              <div className="mt-2 grid grid-cols-2 gap-2">
                <a href={BOOKING_URL} target="_blank" rel="noopener noreferrer" className="flex min-h-11 items-center justify-center rounded-lg border border-purple-400/30 bg-purple-500/10 px-2 py-2 text-center text-[11px] font-semibold text-purple-200 hover:bg-purple-500/20">Google Calendar</a>
                <a href={directWhatsAppUrl} target="_blank" rel="noopener noreferrer" className="flex min-h-11 items-center justify-center rounded-lg border border-emerald-400/30 bg-emerald-500/10 px-2 py-2 text-center text-[11px] font-semibold text-emerald-200 hover:bg-emerald-500/20">WhatsApp +966511493209</a>
              </div>
            </div>
          </motion.section>
        )}
      </AnimatePresence>
      <button ref={launcherRef} type="button" onClick={isOpen ? closeChat : () => openChat()} aria-label={isOpen ? 'Close portfolio assistant' : 'Open portfolio project assistant'} aria-expanded={isOpen} className="group flex items-center gap-3 rounded-full border border-cyan-400/30 bg-[#0E1322]/95 p-2 pr-4 text-white shadow-[0_14px_45px_rgba(0,0,0,0.42),0_0_24px_rgba(34,211,238,0.12)] backdrop-blur-xl"><span className="flex h-10 w-10 items-center justify-center rounded-full border border-cyan-400/50 text-xs font-black text-cyan-100">SH</span><span className="text-left"><span className="block text-xs font-semibold">Ask Shaikh's project assistant</span><span className="mt-0.5 block text-[11px] text-cyan-300">Questions · Projects · Contact</span></span></button>
    </div>
  );
}
